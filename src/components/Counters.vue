<template>
  <div class="counters">
    ...
    <div class="action-bar">
      <el-button type="primary" v-if="!currentNode" v-on:click="() => setCurrentNode(undefined)" icon="el-icon-plus" plain>Add counter</el-button>
    </div>
    <div class="node-manipulation rounded" v-if="currentNode" @keyup.enter="saveCurrentNode">
      <div class="input-row col5050">
        <el-input placeholder="Counter name" v-model="currentNode.name">
          <template slot="prepend">Counter name</template>
        </el-input>
      </div>
      <div class="input-row col5050">
        <el-input placeholder="Reset threshold" v-model="currentNode.threshold">
          <template slot="prepend">Threshold</template>
        </el-input>
      </div>
    ...
    <div class="list-container">
      <div v-for="node in counters" v-bind:key="node.id" class="list-row pointer">
        <div class="col5050">
          <span>{{node.name}}</span>
        </div>
  ...
  methods: {
    ...
    saveCurrentNode: function () {
      ...
      if (this.currentNode.id === -1) {
        ...
        this.saveCounter(nodeToInsert)
      }
      else {
        this.editCounter(JSON.parse(JSON.stringify(this.currentNode)))
    ...
    deleteCurrentNode: function () {
      if (!this.currentNode.askDeleteConfirmation) {
        ...
        this.deleteCounter(JSON.parse(JSON.stringify(this.currentNode)))
        this.currentNode = undefined
      }
    },
    deleteNode: function (node) {
      if (!node.askDeleteConfirmation) {
        ...
        this.deleteCounter(JSON.parse(JSON.stringify(node)))
      }
    },
    ...
    validateNode: function() {
      ...
      if (this.currentNode.threshold === 0) {
        localMessage += 'Threshold '
        error = true
      }
